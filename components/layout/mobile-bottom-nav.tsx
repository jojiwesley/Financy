        ...
        {navItems.map((item, index) => {
          if (item.type === "button") {
            return (
              <div
                key="quick-add"
                className="flex flex-1 items-center justify-center -mt-8"
              >
                <button
                  onClick={item.onClick}
                  className="flex h-12 w-12 items-center justify-center rounded-full bg-linear-to-br from-primary to-violet-600 text-white shadow-lg shadow-primary/30 transition-transform active:scale-95 border-2 border-background"
                >
