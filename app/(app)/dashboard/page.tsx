...
export default async function DashboardPage() {
  ...
  const [
    ...
    { data: billRowsRaw },
    { data: allTxRaw },
    { data: allAccountTxRaw },
  ] = await Promise.all([
    // Transações do mês atual: usa apenas a data real da transação para os KPIs
    supabase
      .from("transactions")
      .select(
        "type, amount, description, date, reference_date, status, categories(name, color)",
      )
      .eq("user_id", user!.id)
      .eq("status", "confirmed")
      .gte("date", firstDay)
    ...
    // Todas as transações vinculadas a contas (para saldo real)
    supabase
      .from("transactions")
  ...
  type TxDash = Pick<
    Tables<"transactions">,
    "type" | "amount" | "description" | "date" | "reference_date" | "status"
  > & {
  ...
  type TxChart = Pick<
    Tables<"transactions">,
    "type" | "amount" | "date" | "reference_date"
  >;
  type BillDash = Pick<
    Tables<"bills">,
    "amount" | "description" | "due_date" | "status"
  >;
  type AccountDash = Pick<
    Tables<"accounts">,
  ...
  const allAccountTx = (allAccountTxRaw as AccountTx[] | null) ?? [];

  // Compute real current balance per account (initial + all income - all expenses)
  const accountBalanceMap = new Map<string, number>();
  for (const tx of allAccountTx) {
    if (!tx.account_id) continue;
    const prev = accountBalanceMap.get(tx.account_id) ?? 0;
  ...
  const expenses = transactions
    ...
    .reduce((s, t) => s + (t.amount ?? 0), 0);
  // Real total balance = sum of (initial balance + account transactions)
  const totalBalance = accounts.reduce(
    (s, a) => s + (a.balance ?? 0) + (accountBalanceMap.get(a.id) ?? 0),
    0,
  );
  const totalPendingBills = bills.reduce((s, b) => s + (b.amount ?? 0), 0);
  ...
  for (const tx of transactions.filter((t) => t.type === "expense")) {
    ...
    catMap.set(key, {
      total: (existing?.total ?? 0) + (tx.amount ?? 0),
      color,
    });
  }
  ...
  const DeltaBadge = ({ delta }: { delta: number | null }) => {
    ...
    return (
      <span
        className={`flex items-center gap-0.5 text-xs font-medium ${positive ? "text-green-600" : "text-red-600"}`}
      >
        {positive ? (
    ...
    <div className="space-y-8">
      ...
      {overdueBills.length > 0 && (
        <div className="flex items-center gap-3 rounded-xl border border-red-500/20 bg-red-500/10 px-4 py-3 text-red-600 dark:text-red-400 backdrop-blur-sm">
          ...
          <p className="text-sm font-medium">
            ...
            {overdueBills.length > 1 ? "s" : ""} vencida
      ...
      <div className="flex flex-col gap-4 md:hidden">
        <MetricCard
          ...
          subValue={
            totalPendingBills > 0
              ? `Projeção: ${fmt(projectedBalance)} · ${fmt(totalPendingBills)} a pagar`
              : `em ${accounts.length} conta${accounts.length !== 1 ? "s" : ""}`
          }
        />
          ...
          <div className="min-w-[85%] snap-center flex">
            <MetricCard
              ...
              trend={
                incomeDelta !== null
                  ? {
                      value: parseFloat(incomeDelta.toFixed(1)),
          ...
          <div className="min-w-[85%] snap-center flex">
            <MetricCard
              ...
              trend={
                expensesDelta !== null
                  ? {
                      value: parseFloat(expensesDelta.toFixed(1)),
                      label: "vs mês anterior",
                    }
                  : undefined
              }
                ...
                transactions.slice(0, 8).map((tx, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p
                        className={`text-sm font-semibold tabular-nums shrink-0 ml-2 ${tx.type === "income" ? "text-green-600" : "text-red-600"}`}
                      >
                        {tx.type === "income" ? "+" : "−"}
                        {fmt(tx.amount ?? 0)}
                      </p>
              ...
              <div className="divide-y divide-border/50">
                {bills.length === 0 ? (
                  ...
                  bills.slice(0, 5).map((bill, idx) => {
                    ...
                    return (
                      <div
                        key={`${bill.description}-${idx}`}
                        className="flex items-center justify-between px-4 sm:px-6 py-3 hover:bg-muted/30 transition-colors"
                      >
                        <div className="flex items-center gap-3 min-w-0">
                          <div
                            className={`shrink-0 rounded-full p-2 ${isOverdue ? "bg-red-100 dark:bg-red-900/30" : "bg-orange-100 dark:bg-orange-900/30"}`}
                          >
                            <CreditCard
                              className={`h-3.5 w-3.5 ${isOverdue ? "text-red-600" : "text-orange-600"}`}
                            />
                          </div>
                          <div className="min-w-0">
                            ...
                            <p
                              className={`text-xs truncate ${isOverdue ? "text-red-500 font-medium" : "text-muted-foreground"}`}
                            >
                        ...
                        <p
                          className={`text-sm font-semibold tabular-nums shrink-0 ml-2 ${isOverdue ? "text-red-600" : "text-orange-600"}`}
                        >
                          {fmt(bill.amount)}
                        </p>
                      </div>
                ...
                {accounts.length === 0 ? (
                  ...
                  accounts.map((acc, idx) => (
                    ...
                    >
                      ...
                      <p
                        className={`text-sm font-semibold tabular-nums ${(acc.balance ?? 0) >= 0 ? "text-foreground" : "text-red-600"}`}
                      >
                        {fmt(acc.balance ?? 0)}
                      </p>
                    </div>
