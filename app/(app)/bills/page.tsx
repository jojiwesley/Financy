...
import { payBill, deleteBill } from "./actions";
...
export default async function BillsPage() {
  ...
  const totalPending = [...pending, ...overdue].reduce(
    (s, b) => s + (b.amount ?? 0),
    0,
  );
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Acompanhe suas contas e vencimentos"
      >
        <Link
          href="/bills/new"
      ...
      {groups.map(({ title, items, highlight }) =>
        items.length > 0 ? (
          <div key={title}>
            <h2
              className={`mb-3 text-base font-semibold ${highlight ? "text-red-600" : ""}`}
            >
              {title}
            </h2>
            <Card>
              <div className="divide-y">
                {items.map((bill) => {
                  ...
                  const deleteAction = deleteBill.bind(null, bill.id);
                  return (
                    <div
                      key={bill.id}
                    ...
                    >
                      ...
                      <span
                        className={`hidden sm:flex items-center gap-1 text-xs font-medium ${cfg?.color ?? "text-muted-foreground"}`}
                      >
                      ...
                      <div className="flex items-center gap-1.5 flex-shrink-0">
                        ...
                        <Link
                          ...
                          className="inline-flex h-7 w-7 items-center justify-center rounded border border-input bg-background hover:bg-accent transition-colors"
                          title="Editar"
                        >
                        ...
                        <form action={deleteAction}>
                          <button
                            type="submit"
                            className="inline-flex h-7 w-7 items-center justify-center rounded border border-input bg-background text-red-500 hover:bg-red-50 hover:border-red-300 transition-colors"
                            title="Excluir"
                          >
