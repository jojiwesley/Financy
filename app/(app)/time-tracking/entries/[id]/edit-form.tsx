...
export function EditEntryForm({ entry }: { entry: TimeEntry }) {
  ...
  const [clockIn, setClockIn] = useState(
    entry.clock_in ? entry.clock_in.slice(0, 5) : "",
  );
  const [lunchStart, setLunchStart] = useState(
    entry.lunch_start ? entry.lunch_start.slice(0, 5) : "",
  );
  const [lunchEnd, setLunchEnd] = useState(
    entry.lunch_end ? entry.lunch_end.slice(0, 5) : "",
  );
  const [clockOut, setClockOut] = useState(
    entry.clock_out ? entry.clock_out.slice(0, 5) : "",
  );
  const [expectedHours, setExpectedHours] = useState(
    String(entry.expected_hours ?? 8),
  );
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <button
        type="submit"
        disabled={isPending}
        className="w-full flex items-center justify-center gap-2 rounded-xl bg-linear-to-r from-primary to-violet-600 py-3 text-sm font-semibold text-white shadow-lg shadow-primary/25 hover:shadow-primary/40 active:scale-[0.98] transition-all duration-300 disabled:opacity-60"
      >
        {isPending ? (
